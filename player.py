...
class Player:
    ...
    def do_turn(self) -> None:
        ...
        # play cards
        want_to_play_card: str = inputWrapper.InputWrapper.input_wrap("Do you want to play a card? (y/n): ")
        while True:
            if want_to_play_card in ["N", "n"]:
                break
            elif want_to_play_card in ["Y", "y"]:
                self.play_card()
                want_to_play_card: str = inputWrapper.InputWrapper.input_wrap("Do you want to play another card? (y/n): ")
            else:
                want_to_play_card: str = inputWrapper.InputWrapper.input_wrap(
    ...
    def play_card(self) -> None:
        ...
        """
        # get card input
        card_name_or_id = inputWrapper.InputWrapper.input_wrap("Enter either Card Name or ID (Q to quit): ")
        while True:
            card_index = -1
            ...
            # failed energy check
                ...
                card_name_or_id = inputWrapper.InputWrapper.input_wrap("You don't have enough energy to play this card: ")
            # wrong input
            else:
                card_name_or_id = inputWrapper.InputWrapper.input_wrap(
                    "Card Name or ID are not matching any cards in hand: ")
        # get area input:
        area_index: int = int(inputWrapper.InputWrapper.input_wrap("Enter Area index: "))
        while True:
            if area_index in [1, 2, 3] and game.Game.instance().is_card_playable_in_area(self._hand[card_index],
                                                                                         area_index):
                break
            else:
                area_index = int(inputWrapper.InputWrapper.input_wrap("Invalid Area number, please enter 1, 2, 3: "))
